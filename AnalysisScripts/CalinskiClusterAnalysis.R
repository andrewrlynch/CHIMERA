...
###LOOP-FILLING VECTORS
clusters <- c()
intraclustermeaneuc <- c() #Vector of average euclidean distances within clusters for each simulation
meanclusterdistance <- c() #Vector of average euclidean distances between karyotypic modes of clusters for each simulation
TemporalCINMetric <- c() #Vector of temporal CIN metrics for each simulation
###LOOPING CLUSTER ANALYSIS
for (nx in distinct(testdf[,c(1,4,8,11,12,36)])[complete.cases(distinct(testdf[,c(1,4,8,11,12,36)])), ]$X.run.number.) {
  print(nx)
  print(nrow(distinct(testdf[,c(1,4,8,11,12,36)])))
  ...
  ###CALCULATE MEAN EUCLIDEAN DISTANCE WITHIN EACH CLUSTER
  ...
  #RATIO CLUSTERS/INTER-CLUSTER DISTANCE
  TemporalCINMetric[nx] <- clusters[nx] / meanclusterdistance[nx]
}
###RECONSTRUCT TEST DATA SET
testdf <- cbind(recast, euc)
testdf1$clusters <- clusters
testdf1$INTRAcluster <- intraclustermeaneuc
testdf1$INTERcluster <- meanclusterdistance
testdf1$CINmetric <- TemporalCINMetric
testdf1 <- arrange(testdf1, CIN.rate, periodicity, selective.pressure)
testdf1$rep <- rep(seq(1:5), 264)
testdf1sb <- subset(testdf1, selective.pressure != 0)
testdf1sb$CIN.rate <- round(testdf1sb$CIN.rate, 4)
...
par(mar = rep(0, 4))
dendrogram <- circlize_dendrogram(dend, dend_track_height = 0.7)
text(0,0.1,toString(round(subset(distinct(testdf[,c(1,4,8,11,12,36)])[complete.cases(distinct(testdf[,c(1,4,8,11,12,36)])), ], X.run.number. == nx)$CIN.rate, 4)), cex = 0.75, font = 2)
text(0,0,toString(subset(distinct(testdf[,c(1,4,8,11,12,36)])[complete.cases(distinct(testdf[,c(1,4,8,11,12,36)])), ], X.run.number. == nx)$selective.pressure), cex = 0.75, font = 2)
text(0,-0.1,toString(subset(distinct(testdf[,c(1,4,8,11,12,36)])[complete.cases(distinct(testdf[,c(1,4,8,11,12,36)])), ], X.run.number. == nx)$periodicity), cex = 0.75, font = 2)
...
plot <- ggplot(testdf1sb, aes(x=log10(selective.pressure), y = INTRAcluster, fill = as.factor(selective.pressure))) + 
  geom_line(group = testdf1sb$rep, lty = 3) +
  geom_point(size = 3, shape = 21) + 
  facet_grid(periodicity ~ CIN.rate) + 
  theme_bw() + 
...
#Analysis heatmaps
p1 <- ggplot(subset(testdf1sb, periodicity == "early" & CIN.rate != 0), aes(x = as.factor(CIN.rate), y = as.factor(selective.pressure), fill = INTERcluster)) + geom_tile()
p2 <- ggplot(subset(testdf1sb, periodicity == "late" & CIN.rate != 0), aes(x = as.factor(CIN.rate), y = as.factor(selective.pressure), fill = INTERcluster)) + geom_tile()
p3 <- ggplot(subset(testdf1sb, periodicity == "constant" & CIN.rate != 0), aes(x = as.factor(CIN.rate), y = as.factor(selective.pressure), fill = INTERcluster)) + geom_tile()

ggarrange(p1,p2,p3, ncol = 3, common.legend = TRUE)

#Getting rid of periodicity
plot <- ggplot(subset(testdf1sb, periodicity == "constant"), aes(x=log10(selective.pressure), y = INTRAcluster, fill = as.factor(selective.pressure))) + 
  geom_point(size = -log(subset(testdf1sb, periodicity == "constant")$CIN.rate), shape = 21) +
  theme_bw() + 
  theme(axis.text = element_text(angle = 90, color = "black")) +
  theme(strip.text.x = element_text(color="black",
                                    face="bold"),
...
#Elbow <- fviz_nbclust(scale(df), kmeans, "wss")
#Silhouette <- fviz_nbclust(scale(df), kmeans, "silhouette")
#View(testdf1sb)

#####
BoxplotSubset <- subset(testdf1sb, periodicity == "constant")
BoxplotSubset <- subset(BoxplotSubset)
ggplot(BoxplotSubset, aes(x = as.factor(selective.pressure), y = 1/CINmetric, fill = as.factor(CIN.rate))) + geom_boxplot(outlier.colour = "black", outlier.fill = "white") + theme(legend.position = "top") + theme_pubr() + labs_pubr() + ylim(0,1.5)

ggplot(BoxplotSubset, aes(x = as.factor(selective.pressure), y = clusters, fill = as.factor(CIN.rate))) + geom_boxplot(outlier.colour = "black", outlier.fill = "white") + theme(legend.position = "top") + theme_pubr() + labs_pubr()
